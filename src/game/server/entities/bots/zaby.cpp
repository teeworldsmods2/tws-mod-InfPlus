...
MACRO_ALLOC_POOL_ID_IMPL(CZaby, MAX_CLIENTS)

CZaby::CZaby(CGameWorld *pWorld)
: CCharacter(pWorld)
{
	m_BotDir = 1;
	m_BotLastPos = m_Pos;
...
bool CZaby::TakeDamage(vec2 Force, int Dmg, int From, int Weapon)
{
...
void CZaby::PlaySound()
{

}

void CZaby::RunAction()
{
...
void CZaby::TickBotAI()
{
	...
	for (int i=0; i<g_Config.m_SvMaxClients; i++)
	{
	    ...
	    if (!pPlayer || !pPlayer->GetCharacter() || pPlayer->IsBot() || pPlayer->GetBotType() == ZOMBIECLASS_ZABY)
            continue;
