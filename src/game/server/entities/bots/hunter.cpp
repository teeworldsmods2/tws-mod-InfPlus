...
MACRO_ALLOC_POOL_ID_IMPL(CHunter, MAX_CLIENTS)

CHunter::CHunter(CGameWorld *pWorld)
: CCharacter(pWorld)
{
	m_BotDir = 1;
	m_BotLastPos = m_Pos;
...
bool CHunter::TakeDamage(vec2 Force, int Dmg, int From, int Weapon)
{
...
void CHunter::PlaySound()
{

}

void CHunter::RunAction()
{
...
void CHunter::TickBotAI()
{
	...
	for (int i=0; i<g_Config.m_SvMaxClients; i++)
	{
	    ...
	    if (!pPlayer || !pPlayer->GetCharacter() || pPlayer->IsBot() || pPlayer->GetBotType() == ZOMBIECLASS_BOOMER)
            continue;
